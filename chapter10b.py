...
class PieVisitorI(ABC):
    @abstractmethod
    def for_bot(self, that: "Bot") -> object:
        pass

    @abstractmethod
    def for_top(self, that: "Top") -> object:
        pass
...
class OccursV(PieVisitorI):
    ...
    def for_top(self, that: Top) -> object:
        if that.t == self.a:
            return 1 + that.r.accept(self)
        else:
            return that.r.accept(self)
